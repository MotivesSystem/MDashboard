import { Avatar, Stack, Typography, useTheme, Box, Grid } from '@mui/material';
    ...
    return (
        <Stack spacing={2} minHeight={370}>
            ...
            <Stack>
                {
                    !loadingYearly ?
                        <Stack columnGap={1} rowGap={1} justifyContent='center' alignItems='center' py={1} minHeight={100}>
                                    ...
                                    dataYearly.map((item, index) => (
                                        <Grid item lg={4} md={6} sm={6} key={index}>
                                            <Stack display={'flex'} direction={'row'} justifyContent='center' alignContent='center' spacing={1} key={`${item?.employee_id}-${index}`} height={'100%'} position={'relative'}>
                                                <Stack display={'flex'} justifyContent={'center'} alignItems={'center'} height={'100%'} width={"100%"} spacing={1} >
                                                    ...
                                                    <LinearProgressWithLabel value={item?.performance_avg} width={"90%"} />
                                                </Stack>
                                            </Stack>
                                        </Grid>
                                    ))
